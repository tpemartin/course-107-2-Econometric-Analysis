...
```{r}
CigarettesSW %<>% as_tibble() ->CigarettesSW  #串接，as_tibble(as開頭的)是想改變他的class或是增加他class的特性
as_tibble(CigarettesSW) -> CigarettesSW #非串接
```

```{r}
CigarettesSW #變tibble的話就不會全部一次跑出來
```
```{r}
CigarettesSW %<>% as_tibble() #表示串接完會再丟回來所以用<> 要先library(magrittr)magrittr底下的串接語法
...
```
```{r}
model1<-log(packs) ~ log(rprice)  #formula(class)也可以先存起來 這樣的model本身是帶有常數項的
...
#這邊還沒有調整一直變異數的公式，所以要用sandwich裡的vcocHC的函數，且設定type
library(sandwich)
library(lmtest)
library(stargazer)
...
```{r}
ols1 #沒有調整的
ols1_coeftest #才是調整完的standard error
#係數值不會變(因為都是做ols正交投射)
```
...
summary(tsls_2iv, vcov = sandwich, diagnostics = TRUE, df=Inf) -> tsls_2iv_tests #第2個多放一個為了能夠做sargen test
...
#Weak test pvalue1.04e-08*** small p reject Ho Ho:weak IV
